...
- **Root cause of Claire's *missing* posts (no code change — operational).** `syncCampaign` drops posts via two filters: the campaign **date window** (`startDate`..`endDate`) and the **hashtag filter** (`filterByHashtags`, only keeps posts whose caption contains a campaign hashtag — "leave empty to track all posts"). Claire has 3 IG Reels + 5 TikToks but only 2 IG + 3 TikTok show because the others' captions don't include the campaign hashtag. **Fix:** edit the campaign → clear the Hashtags field (and confirm the date range covers the post dates) → re-sync.
