...
- **Dormant-handle cadence**: a handle whose newest tracked post is 14+ days old (or that has been scraped before and never produced a post — dead/SB'd handles) drops from nightly to every ~3 days. A dormant video that suddenly goes viral still shows within ≤3 days via the shallow re-check, and the weekly deep pass still refreshes its exact count. This is the "still track older creators, just less often" tier.
- **Per-creator manual sync finally rate-limited**: it had NO limit — every click deep-scraped every handle (IG = 2 actors ×60 results). Now the same 30-min freshness window as the campaign button; response reports `freshSkipped`.
- **Visibility**: sync summary carries `apifyBudget {mode, usedUsd, limitUsd}`, `dormantSkips`, `budgetSkips`; SyncHealthBanner shows a budget-pacing note whenever the last sync ran reduced (even with zero failures) — ahead-of-pace spending is visible BEFORE it becomes a wall of 403s.
- Immediate unblock still manual: raise the limit at console.apify.com → Billing (cycle resets ~Aug 22). Recommendation: set the monthly limit to ~$50–75; with these guardrails the pacing modes will make whatever is set last the cycle.
- Tested: `npm test` 73/73 (6 new dormancy/budget tests), `npx next build` green. No schema change.
...
- Follow-up (same day, per Jackie): "Viral posts" hero stat now explains itself inline — shows the actual view threshold and the rule ("over X views — 3× the typical post this period"). The report's viral definition is relative (3× the window's median views, mean fallback when median is 0), intentionally separate from the per-campaign pacing viral threshold.
- Tested: `npm test` 57/57, `npx next build` green.
...
- **Freshness skip**: a handle successfully scraped in the last 6h is skipped by the cron (dedupes handles shared across campaigns in one run and re-fired crons); the manual Sync Data button uses a 30-min window (a click is an explicit ask for fresh data; back-to-back syncs of campaigns sharing handles no longer double-scrape). Failures never count as fresh — anything that errored retries at full cadence and depth. The per-creator manual sync stays full-depth and records its result so the cron doesn't redo it within the window.
- **Schema change**: new `handle_sync_states` table (per platform+handle `lastSuccessAt` / `lastDeepAt`), migration `20260722160000_handle_sync_state`, additive only. Sync summary now records `freshSkips` (skips are not failures — they don't trip the banner).
- Token situation: root cause of the Jul 22 banner is confirmed as the Apify account's monthly usage hard limit (403 on every launch). The new `APIFY_TOKEN` is present in the Claude cloud environment, but this sandbox's network policy blocks api.apify.com, so it could not be validated from here — and it must ALSO be set in Vercel → tracker-app → Settings → Environment Variables (Production) + redeployed to take effect in prod; the Vercel project config was last updated Jul 20, i.e. before the token swap, so it likely still needs doing.
- Tested: `npm test` 57/57 (10 new planner tests), `npx next build` green (migrate deploy skipped locally — no DB URL here).
...
- **Report "Views over time" kept as a cumulative trend, but re-sourced from `Post` (`src/lib/reports/report-data.ts`, `src/components/reports/client-report.tsx`).** Per Jacqueline's call ("trend is better because not everyone is posting daily"), we kept the climbing cumulative line rather than switching to a daily-bucketed chart. It now buckets the window's posts by publish day and accumulates, so the line ends exactly at the hero **Total views** (it reconciles). Previously it read `CampaignDailyMetric`, whose rows are cumulative campaign-to-date snapshots keyed by *sync* date — unusable as a per-day series.
- **Week-over-week deltas re-sourced from `Post` (same file).** All four WoW figures (posts / views / engagements / viral) now compare this period's posts vs. the previous period's posts, so they reconcile with the rest of the report. Removed the last dependence on `CampaignDailyMetric` from the report engine.
- **Charts tab "Posts Per Day" / "Views & Engagement Over Time" fixed (`src/app/(admin)/charts/page.tsx`, `src/components/charts/campaign-charts-client.tsx`).** Per-campaign series is now computed straight from `Post` (bucketed by `postedAt` over the last 90 days), respecting team visibility. This is the **131-posts bug**: the old code read `CampaignDailyMetric.totalPosts`, which the sync writer was filling with the cumulative campaign-to-date count. All three charts now read as true per-day activity (Jacqueline's choice — consistent across the tab). Dropped the unused `activeCreators` field from the client component.
- **Sync writer stops recording cumulative totals (`src/lib/social/sync.ts`).** `updateCampaignDailyMetrics` now scopes its aggregate to posts *published* that calendar day (`postedAt` within `[date, date+1)`), so `CampaignDailyMetric` rows are true daily activity going forward. **No backfill** (Jacqueline's call) — historical rows stay cumulative, so the campaign **Overview** "daily views" chart (the one remaining reader of that table) self-heals only as new syncs land.
- **Tested:** `next build` passes clean (exit 0); all routes compile. Verified the report renders end-to-end against the temporary `/reports/preview` mock (HTTP 200) — hero (33 posts / 6.1K views / 2.3% eng / viral threshold 240), trend section, platform leaderboard, top posts, top captions, posting-time, caption-length, creator breakdown, WoW "vs prev period", Download PDF all present. **Live number-accuracy NOT yet verified** — gated on Vercel/DB access; preview is hand-built mock data.
- **Note:** the temporary `src/app/reports/preview/page.tsx` (mock-data preview for demoing to a coworker) is intentionally kept in this PR at Jacqueline's request until Vercel access lands; delete in a follow-up.
...
- **Client report — Phase 1 (`src/lib/reports/report-data.ts`, `src/components/reports/client-report.tsx`, `src/app/(admin)/campaigns/[campaignId]/reports/view/page.tsx`, `src/app/(admin)/clients/[teamId]/report/page.tsx`).** On-site, print-to-PDF report rendered from real `Post` / `CampaignDailyMetric` data — works at two scopes from one engine: per-campaign and per-client (aggregates every campaign on the team). `computeReport(scope)` returns the full KPI payload; `ClientReport` renders it. Covers the data-derivable KPIs from Jacqueline's 22-KPI spec: **Tier 1** hero (posts shipped, total views, engagement rate `(likes+comments+shares+saves)/views`, viral posts where views ≥ 3× median); **Tier 2** top viral posts (top 10), top captions by eng rate (view floor to avoid low-sample noise), platform leaderboard; **Tier 3** underperforming platform; **Tier 4** WoW deltas (posts / views / engagements / viral); **Tier 5** posting-time-by-day, caption-length sweet-spot, views trend over time. Hook/format/theme KPIs (#5, #9, #10, #12, #17, #18, #21) are deferred to Phase 2 (AI content classification → `ContentTraits`). Print CSS isolates `#client-report` for a clean PDF and hides the Download button. Entry points: "View full report" card on `/campaigns/[id]/reports`, "View report" header link on `/clients/[teamId]`.
- **Sync post cap raised 30 → 60 per platform (`src/lib/social/apify.ts`).** Both scraper fns defaulted to `limit = 30`; neither the campaign/cron sync nor the per-creator button passed an explicit limit, so each creator only ever pulled their 30 most-recent TikToks + 30 most-recent IG posts within the window. Per Jacqueline, creators post 30–40 (min) to 60 (max) videos per campaign, so 30 risked clipping the oldest in-window posts on busy accounts. Bumped both defaults to 60. Apify cost scales ~linearly with results, so each scrape can now cost up to 2× — still small at this creator count (TikTok $0.20/1K, IG ~$1/1K).
- **Architecture note — no schema change.** Per-client + per-campaign reports are auth-gated on-site view pages parameterized by id (campaign: `campaignVisibilityWhere`; client: agency-wide access OR own-team). The public `/reports/[slug]` slug-shared rich report is a separate future upgrade (would need a `WeeklyReportConfig` change for client scope).
- **Tested:** `npm run build` clean on Node 20.19. New `/creator-posts` route compiles (2.43 kB); `/creators/[creatorId]` grew to 9.62 kB with the chart; `/clients/[teamId]/report` and `/campaigns/[id]/reports/view` compile. `next lint` clean of all unused-var/TS warnings. recharts `Tooltip` formatters follow the repo's untyped `(value) => [...]` + `Number(value)` pattern (the typed-param form fails the build).
